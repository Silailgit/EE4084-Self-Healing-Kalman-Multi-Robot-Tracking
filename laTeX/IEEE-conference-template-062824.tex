...
Reliable tracking in indoor environments remains a challenging problem. Sensor measurements are often corrupted by noise, while temporary communication failures or sensor outages may result in missing observations. These issues become even more critical when multiple robots operate simultaneously in the same environment, as tracking errors may affect navigation performance and increase the risk of collisions.

State estimation techniques based on Kalman filtering have been widely used for target tracking and robot localization because of their computational efficiency and robustness against measurement noise. However, maintaining reliable estimates during measurement losses remains an important challenge. Furthermore, nonlinear filtering approaches such as the Extended Kalman Filter (EKF) are frequently employed when the underlying system dynamics cannot be adequately represented by linear models.
...
The Kalman Filter (KF) is employed to estimate the position of each robot from noisy measurements. The filter operates recursively using two main stages: prediction and correction.
...
The Kalman Filter provides an efficient solution for estimating robot trajectories under noisy measurement conditions.

\subsection{Extended Kalman Filter}

In addition to the standard Kalman Filter, an Extended Kalman Filter (EKF) was implemented to investigate the effect of nonlinear motion modeling.
...
The EKF approximates the nonlinear system using first-order linearization through the Jacobian matrix. The resulting prediction and update procedures follow the standard EKF formulation.
...
Tracking performance was evaluated using the Root Mean Square Error (RMSE) between the estimated robot positions and the corresponding ground-truth positions obtained from the simulator.
...
\bibitem{welch}
G. Welch and G. Bishop, ``An Introduction to the Kalman Filter,'' University of North Carolina at Chapel Hill, TR 95-041, 2006.

\bibitem{barshalom}
Y. Bar-Shalom, X. R. Li, and T. Kirubarajan, \emph{Estimation with Applications to Tracking and Navigation}. New York, NY, USA: Wiley, 2001.

\bibitem{simon}
D. Simon, \emph{Optimal State Estimation: Kalman, H Infinity, and Nonlinear Approaches}. Hoboken, NJ, USA: Wiley, 2006.

\bibitem{thrun}
S. Thrun, W. Burgard, and D. Fox, \emph{Probabilistic Robotics}. Cambridge, MA, USA: MIT Press, 2005.

\bibitem{maybeck}
P. S. Maybeck, \emph{Stochastic Models, Estimation, and Control}, Vol. 1. New York, NY, USA: Academic Press, 1979.

\bibitem{webots}
O. Michel, ``Webots: Professional Mobile Robot Simulation,'' \emph{International Journal of Advanced Robotic Systems}, vol. 1, no. 1, pp. 39--42, 2004.

\bibitem{ekf}
M. S. Grewal and A. P. Andrews, \emph{Kalman Filtering: Theory and Practice Using MATLAB}, 4th ed. Hoboken, NJ, USA: Wiley, 2015.

\bibitem{multirobot}
L. E. Parker, ``Current State of the Art in Distributed Autonomous Mobile Robotics,'' in \emph{Distributed Autonomous Robotic Systems 4}. Tokyo, Japan: Springer, 2000, pp. 3--12.

\bibitem{tracking}
S. Blackman and R. Popoli, \emph{Design and Analysis of Modern Tracking Systems}. Norwood, MA, USA: Artech House, 1999.
